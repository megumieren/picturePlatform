...
import com.wjq.wjqpicturebackend.utils.color.ColorSimilarUtils;
...
//                    ErrorCode.NO_AUTH_ERROR);
        ...
        if (spaceId == null) {
            //如果spaceId为空，则上传到public目录下并且按照用户id划分目录
...
//        }
        ...
        Color sourceColor = Color.decode(picColor);
        //查询该空间下有主色调的图片
        ...
        return pictureList.stream().sorted(Comparator.comparingDouble(picture -> {
                    ...
                    if (StrUtil.isBlank(hexColor)) {
                        return Double.MAX_VALUE; //没有主色调的放在最后
                    }
                    Color targetColor = Color.decode(hexColor);
                    return ColorSimilarUtils.calculateSimilarity(sourceColor, targetColor);  //值越小，相似度越大
                }))
...
//        checkPictureAuth(loginUser, picture);
        ...
        BeanUtils.copyProperties(createPictureOutPaintingTaskRequest, createOutPaintingTaskRequest);//必须用此方法
        //创建任务
